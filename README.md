...
13. Copy the logged in user's session token.
...
#### Attack phase
6. Perform a stored XSS attack as outlined above but paste in this in the comment section & press submit:
...
7. Head over to the the pricing page, then to the reviews page and back to the pricing page. 
...
### Comments
The attacker would know how to construct a malicious POST request to the delete-endpoint by by observing and analyzing the traffic leaving the browser when making a regular delete request from the vicitm website.
